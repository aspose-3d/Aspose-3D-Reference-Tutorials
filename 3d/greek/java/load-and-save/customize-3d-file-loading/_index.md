...
# Flip Coordinate System – Προσαρμόστε τη φόρτωση αρχείων 3D σε Java με το Aspose.3D
...
- **Μπορώ να φορτώσω υλικά ταυτόχρονα;** Ναι—χρησιμοποιείτε `setEnableMaterials(true)` για αρχεία OBJ.
- **Απαιτείται άδεια για παραγωγή;** Απαιτείται έγκυρη άδεια Aspose.3D για μη‑δοκιμαστικές εργασίες.

## Τι είναι το «σύστημα συντεταγμένων αναστροφής»;
Η αναστροφή του συστήματος συντεταγμένων αλλάζει τον προσανατολισμό των αξόνων που χρησιμοποιεί το εισαγόμενο μοντέλο. Αυτό είναι απαραίτητο όταν το αρχείο προέλευσης έχει διαφορετική χειρονομία (δεξιόχειρο vs. αριστερόχειρο) από τη μηχανή απόδοσής σας, αποτρέποντας την εμφάνιση των μοντέλων ως κατοπτρισμένα ή ανεστραμμένα.

## Γιατί να προσαρμόσετε την εισαγωγή 3D;
...
## Πώς να προσαρμόσετε την εισαγωγή 3D με το LoadOptions

Παρακάτω θα βρείτε βήμα‑βήμα αποσπάσματα κώδικα που δείχνουν πώς να ενεργοποιήσετε την επιλογή **αναστροφή του συστήματος συντεταγμένων** για κάθε υποστηρικτική μορφή. Τα αποσπάσματα είναι έτοιμα για αντιγραφή στο έργο σας· απλώς αντικαταστήστε το `"Your Document Directory"` με την πραγματική διαδρομή των πόρων σας.
...
### Βήμα 8: Προσαρμόστε τη φόρτωση αρχείων FBX (Προαιρετικό)
...
- **Το μοντέλο εμφανίζεται κατοπτρισμένο μετά τη φόρτωση** – αλλά ότι το `setFlipCoordinateSystem(true)` είναι ορισμένο για τη σωστή μορφή.
...
- **Οι συντεταγμένες υφής είναι ανάποδες** – Για glTF, ίσως `setFlipTexCoordV(true)` εκτός από την αναστροφή των αξόνων.
- **Σφάλματα “αρχείο δεν βρέθηκε”** – Προσθέστε τον φάκελο που περιέχει εσωτερικές πόρους (υφές, βοηθητικά αρχεία) στο `loadOpts.getLookupPaths()`.

## Συμπέρασμα

Αξιοποιώντας το `LoadOptions` του Aspose.3D, μπορείτε να **αναστρέψετε το σύστημα συντεταγμένων** και να **προσαρμόσετε την εισαγωγή 3D** για σχεδόν κάθε κύρια μορφή. Αυτό το επίπεδο ελέγχου εξαλείφει την ανάγκη για μεταγενέστερα scripts επεξεργασία και εξασφαλίζει ότι οι πόροι σας αποδίδονται σωστά από την πρώτη φορά.
